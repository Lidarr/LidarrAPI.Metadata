...
WHERE (to_tsvector('mb_simple', artist.name) @@ plainto_tsquery('mb_simple', %s) OR to_tsvector('mb_simple', artist.sort_name) @@ plainto_tsquery('mb_simple', %s) OR artist.name = %s)
ORDER BY
